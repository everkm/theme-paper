...
/** Mirrors youlog `youlog_lib/widgets/keyword-highlighter` */
...
function initKeywordHighlighter(containerSelector: string): void {
  const container = document.querySelector(containerSelector);
  if (!container) return;
  setupKeywordHighlighter(container as HTMLElement);
}

function installKeywordHighlighter(containerSelector: string): void {
